...
function refreshAverage() {
    ...
    if (count > 0) {
        ...
        // var steps = [1, 2, 3, 5, 8, 13, 21];
        // var closest = steps.reduce(function(prev, curr) {
        // return (Math.abs(curr - average) <= Math.abs(prev - average) ? curr : prev);
        // });
        // $averageResult.text(closest + " (Average: " + average.toFixed(2) + ")");
        $averageResult.text(average.toFixed(1));
    } else {
        $averageResult.text('?');
    }
}
...
function vote(card) {
    if ($(card).hasClass('selected')) {
        ...
        if (!number) {
            number = $(card).children('input.card-text').val();
        }
