...
var express = require('express'),
    ...
    port = process.env.PORT || argv.port || config.port;
...
io.sockets.on('connection', function(socket) {
    ...
    socket.on('signIn', function(data, fn) {
        ...
        socket.broadcast.in(requestedGame).emit('userSignedIn', { 'nickname': client.nickname, 'sid': client.sid, 'mode': client.mode });
    ...
    socket.on('vote', function(data, fn) {
        /* Check to make sure the vote provided is positive number */
        if (config.points.indexOf(data.number) < 0) {
            if (!isNumeric(data.number) || data.number <= 0) {
                return false;
            }
        }
